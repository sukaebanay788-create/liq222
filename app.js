...
function initChart() {
  ...
  state.candleMarkersApi = LightweightCharts.createSeriesMarkers(state.candleSeries, []);
  state.candleSeries.priceScale().applyOptions({ autoScale: true, mode: 0 });

  state.ema65Series = state.chart.addSeries(LightweightCharts.LineSeries, { color: '#a0a4ab', lineWidth: 1, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false, autoscaleInfoProvider: () => null });
  state.ema125Series = state.chart.addSeries(LightweightCharts.LineSeries, { color: '#a0a4ab', lineWidth: 1, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false, autoscaleInfoProvider: () => null });
  state.ema450Series = state.chart.addSeries(LightweightCharts.LineSeries, { color: '#e0e3e8', lineWidth: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false, autoscaleInfoProvider: () => null });
...
function updateMarkersOnChart() {
  if (state.candleMarkersApi) state.candleMarkersApi.setMarkers(state.liquidationMarkers);
}
